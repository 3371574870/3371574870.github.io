...
function run() {
    ...
    for (var k = 0; k < num; k++) {
        ...
        ran.push(colors[Math.round((k / colors.length - Math.floor(k / colors.length)) * colors.length)]);
    }
...
function draw() {
    ...
    for (var j = 0; j < num / 2; j++) {
        draw_flower(i, ran[j * 5], ran[j * 5 + 1], ran[j * 5 + 2], ran[j * 5 + 3], ran[j * 5 + 4]);
    }

    for (var m = num / 2 + 1; m < num; m++) {
        draw_flower_2(i, ran[m * 5], ran[m * 5 + 1], ran[m * 5 + 2], ran[m * 5 + 3], ran[m * 5 + 4]);
    }
...
function draw_petal(n, x_st, y_st, s, ang) {
    ...
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.quadraticCurveTo( - 1 * s * n_1, s * n_1, s * 2 * n_1 * (n_1 - 1), s * 2 * n_1);
    ctx.moveTo(0, s * 2);
    ctx.quadraticCurveTo(s * n_2, s * (2 - n_2), -1 * s * 2 * n_2 * (n_2 - 1), -1 * s * 2 * (n_2 - 1));
    ctx.stroke();
    ctx.closePath();
...
function draw_petal_2(n, x_st, y_st, s, ang) {
    ...
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.quadraticCurveTo( - 0.6 * s * n_21, s * n_21 / 2, 0.4 * s * n_21 * (n_21 - 2), s * n_21);
    ctx.moveTo( - 0.4 * s, s);
    ctx.quadraticCurveTo(s * (0.2 * n_22 - 0.4), s * (0.2 * n_22 + 1), -0.4 * s * (1 - n_22), -0.15 * n_22 * s + s);
    ctx.moveTo(0, 0.85 * s);
    ctx.quadraticCurveTo(s * 0.2 * n_23, 0.35 * s * n_23 + 0.85 * s, 0.4 * s * n_23, 0.15 * s * n_23 + 0.85 * s);
    ctx.moveTo(0.4 * s, s);
    ctx.quadraticCurveTo(s * (0.2 * n_24 + 0.4), s * (1 - n_24 / 2), s * 0.4 * (1 - n_24 * n_24), s * (1 - n_24));
    ctx.stroke();
    ctx.closePath();
    ctx.rotate(ang * Math.PI / 180);
    ctx.translate( - 1 * x_st, -1 * y_st);
